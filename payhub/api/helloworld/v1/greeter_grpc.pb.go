...
func NewPaymentSerivceClient(cc grpc.ClientConnInterface) PaymentSerivceClient {
	return &paymentSerivceClient{cc}
}

func (c *paymentSerivceClient) CreatePaymentOrder(ctx context.Context, in *PaymentCreateRequest, opts ...grpc.CallOption) (*PaymentCreateReply, error) {
	out := new(PaymentCreateReply)
	err := c.cc.Invoke(ctx, PaymentSerivce_CreatePaymentOrder_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// UnimplementedPaymentSerivceServer must be embedded to have forward compatible implementations.
type UnimplementedPaymentSerivceServer struct {
}
...
// UnsafePaymentSerivceServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to PaymentSerivceServer will
// result in compilation errors.
type UnsafePaymentSerivceServer interface {
	mustEmbedUnimplementedPaymentSerivceServer()
}

func RegisterPaymentSerivceServer(s grpc.ServiceRegistrar, srv PaymentSerivceServer) {
	s.RegisterService(&PaymentSerivce_ServiceDesc, srv)
}

func _PaymentSerivce_CreatePaymentOrder_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(PaymentCreateRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(PaymentSerivceServer).CreatePaymentOrder(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: PaymentSerivce_CreatePaymentOrder_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(PaymentSerivceServer).CreatePaymentOrder(ctx, req.(*PaymentCreateRequest))
	}
	return interceptor(ctx, in, info, handler)
}
